...
(define (bsloop c stock strike time interest volatility)
  (define (helper timestep end)
    (if (> timestep end)
        (begin
          (set! x (flatten x))
          (set! y (flatten y))
          (display (plot (lines (map vector x (map (lambda (x) (randr x)) y)) #:color 'red)))
          (set! x '())
          (set! y '()))
        (begin
          (set! y (cons y (black-scholes 'p stock strike 220 timestep .05 .1)))
          (set! x (cons x timestep))
          (helper (+ timestep .01) end))))
  (helper 0.01 time))

(bsloop 'c 100 70 1 .1 .1)
(bsloop 'p 100 70 1 .1 .1)

(bsloop 'c 100 100 1 .1 .1)
(bsloop 'p 100 100 1 .1 .1)

(bsloop 'c 100 130 1 .1 .1)
(bsloop 'p 100 130 1 .1 .1)

(bsloop 'c 200 220 1 .05 0.1)
(bsloop 'p 200 220 1 .05 0.1)
